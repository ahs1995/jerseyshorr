...
function CategorySidebar() {
  const { showCategorySide, setShowCategorySide } = useMenu();
  return (
    <aside
      className={`absolute left-20 top-0 z-[100000] h-full w-[80vw] bg-[#fff] shadow-md duration-300 ease-in-out ${
        showCategorySide ? "translate-x-0" : "-translate-x-100"
      }`}
    >
      <div className="relative py-6 text-primary-900">
        ...
        <div>
          <ArrowLeftIcon
            ...
            onClick={() => setShowCategorySide(false)}
          />
        </div>
