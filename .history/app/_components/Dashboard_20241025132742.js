  ...
  return (
    <div className="grid grid-cols-1 gap-8 md:grid-cols-12 md:gap-0 md:shadow-2xl">
      ...
      <div className="mx-auto mt-4 p-2 md:mt-0 md:p-4">
        <button
          onClick={handleDeleteAccount}
          className="w-full rounded-lg px-4 py-2 text-left text-xs text-accent-400 transition-colors hover:underline md:hidden"
        >
          Delete your account
        </button>
      </div>
      {deleteAccount && (
        <div className="z-1000000000 absolute left-[50%] top-[50%]">
          <DeleteAlertPopup />
        </div>
      )}
    </div>
