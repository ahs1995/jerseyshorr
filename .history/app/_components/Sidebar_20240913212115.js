...
function Sidebar({ toggleSidebar, onOpen }) {
  return (
    <aside
      className={`absolute left-0 top-10 z-[100000] h-full w-[350px] bg-[#fff] shadow-md transition-transform duration-500 ${toggleSidebar ? "translate-x-0" : "-translate-x-full"}`}
    >
      <div className="relative py-6 text-primary-900">
        {/* exit icon */}
        <div className="absolute right-3">
          <XMarkIcon
            className={`h-6 w-6 cursor-pointer text-primary-800 ${toggleSidebar ? "-translate-x-full" : "-translate-x-0"}`}
          />
        </div>
        {/* category list */}
