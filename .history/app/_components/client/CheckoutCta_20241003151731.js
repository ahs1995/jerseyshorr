...
function CheckoutCta() {
  ...
  return (
    <div
      className={`bg-white-50 px-4 pt-4 ${openLogin || openCoupon ? "pb-4" : ""}`}
    >
      {/* text contents */}
      <div className="mb-8 flex flex-col gap-2 text-center text-xs text-primary-500">
        <h5>returning customer? Please login below</h5>
        <h5>You can apply coupon to avail extra discount</h5>
      </div>
      <div className="grid grid-cols-[1fr_1fr] gap-4 px-2">
        <div>
      ...
      <div
        className={clsx(
          "transition-max-height mt-6 overflow-hidden border-2 border-dashed border-primary-100 bg-[#fff] duration-500 ease-in-out",
      ...
      <div
        className={clsx(
          "transition-max-height flex flex-col gap-6 overflow-hidden border-2 border-dashed border-primary-100 bg-[#fff] duration-500 ease-in-out",
