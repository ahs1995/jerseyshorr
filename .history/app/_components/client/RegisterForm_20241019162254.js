...
function RegisterForm() {
  ...
  const { mutate, isLoading: isRegistering } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setMessage(
        "User is successfully registered! Please follow the link in your email to verify yout account.",
      );
    },
  ...
  return (
    <div>
      {message && (
        <div className={isRegistered===true ? "bg-primary-900 text-primary-50" ? isRegistered === false ? "bg-accent-100 text-accent-800": "" }>
          <p>{message}</p>
        </div>
      )}
