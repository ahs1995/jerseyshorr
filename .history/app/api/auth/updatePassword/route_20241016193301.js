...
export async function POST(req) {
  try {
    ...
    if(!currentUser) {
      return new Response(JSON.stringify({status:"Fail", message: "You are not logged in. Please login to get access"}), {status:401});
    }
...
if(!(await currentUser.checkPassword(currentPassword, currentUser.password))) {
  return new Response(JSON.stringify({status: "Fail", message: "Your current password is wrong"}),{status: 401})
}
